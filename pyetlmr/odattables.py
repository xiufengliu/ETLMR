...
import pyetlmr
from pyetlmr.FIFODict import FIFODict

__author__ = "Christian Thomsen, Xiufeng Liu"
...
class Dimension(object):
        ...
        """Update a single row in the dimension table.
           ...
           Arguments:
            - row: a dict which must contain the key for the dimension.
              ...
              the value of row[a] for each a in row.
            - namemapping: an optional namemapping (see module's documentation)
        """
	...
	if self.seq_socket==None: # Establish the connection
	    try:
		self.seq_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		remote_ip = socket.gethostbyname('localhost') #It should changed to for the master node, here hardcode for temporarily.
	    except socket.error, socket.gaierror:
		print 'Failed to create socket'
		sys.exit()
	    self.seq_socket.connect((remote_ip , 8888))	
	    
	try :
	    self.seq_socket.sendall(self.name)
	except seq_socket.error:
	    pass
	nextid = self.seq_socket.recv(256)         
	if nextid:
	    self.__maxid = int(nextid)
	else:
	    self.__maxid = self.__maxid + 1
        return self.__maxid

    def endload(self):
	if self.seq_socket!=None:
	    try:
		self.seq_socket.sendall('END')
		self.seq_socket.close()
	    except:
		pass
	    #
        """Finalize the load."""
...
class CachedDimension(Dimension):
    ...
    def __init__(self, name, key, attributes, lookupatts=(), 
        ...
        if prefill:
            if cachefullrows:
                ...
                # select all key and all attributes
                sql = "SELECT %s FROM %s" % (", ".join(self.all), name)
            else:
                # select key and lookup attributes
                sql = "SELECT %s FROM %s" % \
    ...
    def _before_update(self, row, namemapping):
        ...
        for att in self.lookupatts:
            if (att in namemapping or att in row):
                ...
                # and thus not if the old value is in the cache... We have
                # to drop the cache completely... :-/
                # FIXME: We could look up the old values for the lookup atts
                # since we have the key. It would then be enough to
                # do a del self.__vales2key[oldvalues]
                self.__vals2key.clear()
                break
    ...
    def _after_insert(self, row, namemapping, newkeyvalue):
        ...
        if self.cacheoninsert:
            ...
            if self.cachefullrows:
                tmp = pyetlmr.project(self.all, row, namemapping)
...
class SlowlyChangingDimension(Dimension):
    ...
    def __init__(self, name, key, attributes, lookupatts, versionatt, 
                 ...
                 srcdateatt=None, srcdateparser=pyetlmr.ymdparser,
                 type1atts=(), cachesize=10000, idfinder=None, defaultidvalue=None,
                 targetconnection=None):
        """Arguments:
           - name: the name of the dimension table in the DW
           - key: the name of the primary key in the DW
           ...
           - fromfinder: a function(targetconnection, row, namemapping) 
             ...
             pyetlmr.today is used as fromfinder. If fromfinder is None 
             and srcdateatt is not None, 
             pyetlmr.datereader(srcdateatt, srcdateparser) is used. 
    ...
    def _before_update(self, row, namemapping):
        ...
        for att in self.lookupatts:
            if (att in namemapping or att in row):
                ...
                # and thus not if the old value is in the cache... We have
                # to drop the cache completely... :-/
                # FIXME: Same fix as in CachedDimension could be used here
                self.keycache.clear()
                break
    ...
    def _after_insert(self, row, namemapping, newkeyvalue):
        ...
        if self.caching:
            self._after_lookup(row, namemapping, newkeyvalue)
            tmp = pyetlmr.project(self.all[1:], row, namemapping)
            tmp[self.key] = newkeyvalue
            self._after_getbykey(newkeyvalue, tmp)
...
class SnowflakedDimension(object):
    ...
    def __init__(self, references, expectboguskeyvalues=False):
        ...
        self.rowlookupsql = "SELECT " + ", ".join(self.allnames) + \
            " FROM " + " NATURAL JOIN ".join(map(lambda d: d.name, self.sfdims))
    ...
    def getbyvals(self, values, namemapping={}, fullrow=False):
        ...
        if not fullrow:
            ...
            attstouse = [a for a in self.allnames \
                             if a in values or a in {}] 
            sqlwhere = " WHERE " + \
                " AND ".join(["%s = %%(%s)s" % (att, att) for att in attstouse])
            self.targetconnection.execute(self.rowlookupsql + sqlwhere, 
                                          values, namemapping)
    ...
    def update(self, row, namemapping={}):
        """Update rows in the participating dimension tables.
           ...
           Arguments:
            - row: a dict which must contain the key for each
              participating dimension.
              The row with this key value is updated such that it takes
              the value of row[a] for each a in row that is also the name
              of an attribute in the table.
            - namemapping: an optional namemapping (see module's documentation)
        """
        res = self._before_update(row, namemapping)
        if res:
...
class BulkFactTable(object):
    ...
    def _insertwithnulls(self, row, namemapping={}):
        ...
        data = [pyetlmr.getstrornullvalue(val, self.nullsubst) \
                for val in rawdata]
        self.__count += 1
...
class SubprocessFactTable(object):
    """Class for addition of facts to a subprocess. This subprocess can, e.g.,
       be a logger or bulkloader. Reads are not supported. """
