#  This file cntains the code for the pygrametl-based solution
...
class Dimension(object):
    ...
    def __init__(self, name, key, attributes, lookupatts=(), 
        ...
        if idfinder is not None:
            ...
            self.targetconnection.execute("SELECT MAX(%(key)s) FROM %(name)s" %\
                                              {'key':key, 'name':name})
    ...
    def getbyvals(self, values, namemapping={}):
        ...
        sql = "SELECT " + ", ".join(self.all) + " FROM " + self.name + \
            " WHERE  AND ".join(["%s = %%(%s)s" % (att, att) for att in attstouse])
    ...
    def insert(self, row, namemapping={}):
        ...
        #for refdim in self.refdims: # Added new attributes  of the referencing dimension keys
        #    row[refdim.key] = refdim.ensure(row, refdim.namemapping)
    ...
    def _getnextid(self, row, namemappings):
        self.__maxid += 1
        return self.__maxid
...
class SlowlyChangingDimension(Dimension):
    ...
    def lookup_type1(self, row, namemapping={}):
    ...
    def ensure(self, row, namemapping={}):
        ...
        keyval = self.lookup_type1(row, namemapping)
        if keyval is None or keyval==self.defaultidvalue:
            ...
            if addnewversion: # type 2
                ...
                if toatt:
                    toattval = str(self.tofinder(self.targetconnection, row, namemapping)).strip("':date ")
...
class SnowflakedDimension(object):
    ...
    def getbyvals(self, values, namemapping={}, fullrow=False):
        ...
        if not fullrow:
            ...
            sqlwhere = " WHERE  AND ".join(["%s = %%(%s)s" % (att, att) for att in attstouse])
...
class FactTable(object):
    ...
    def __init__(self, name, keyrefs, measures=(), targetconnection=None):
        ...
        self.lookupsql = "SELECT " + ",".join(self.all) + " FROM " + name + \
            " WHERE " + " AND ".join(["%s = %%(%s)s" % (k, k) for k in self.keyrefs])
